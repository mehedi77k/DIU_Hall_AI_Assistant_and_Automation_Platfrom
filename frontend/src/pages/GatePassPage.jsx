...
/*
 * Converts backend UTC timestamps to Bangladesh time.
 *
 * New records:
 *   2026-07-10T15:26:08+00:00
 *
 * Old records without timezone:
 *   2026-07-10T15:26:08
 *
 * Old timezone-less records are treated as UTC by adding "Z".
 */
function formatDateTime(value) {
  ...
  try {
    ...
    const normalizedValue = containsTimezone
      ? rawValue
      : `${rawValue}Z`
...
export default function GatePassPage() {
  ...
  const loadGatePasses = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/gate-passes`,
        {
          headers: {
      ...
      setError(
        'Backend server is not responding. Please check backend container.',
      )
    }
  }
  ...
  useRealtimeRefresh('gate_pass.changed', () => {
    loadGatePasses()
  })
  ...
  const handleAdminAction = async (
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/gate-passes/${gatePassId}/${action}`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setSuccess(
        `Gate pass ${action}d successfully.`,
      )
        ...
        <div className="card">
          {user?.role === 'student' ? (
            <>
              ...
              {!user.signature_image_path && (
                ...
                >
                  ...
                  Admin approval PDF will not be generated
                  until your signature is uploaded from My
                  Profile.
                </div>
              )}
            ...
            <>
              ...
              <p>
                Administrative users cannot submit gate
                pass requests.
              </p>
