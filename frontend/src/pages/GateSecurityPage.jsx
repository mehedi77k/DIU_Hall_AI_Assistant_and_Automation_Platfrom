...
export default function GateSecurityPage() {
  ...
  useRealtimeRefresh('gate_pass.changed', (message) => {
    const changedVerificationId = message?.payload?.verification_id

    if (activeVerificationId && (!changedVerificationId || changedVerificationId === activeVerificationId)) {
      verifyGatePass(activeVerificationId)
    }
  })
